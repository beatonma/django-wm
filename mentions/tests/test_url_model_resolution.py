...
class GetModelForUrlTests(TestCase):
    ...
    def setUp(self):
        ...
        MentionableTestModel.objects.create(stub_id=self.stub_id, slug=self.slug)
    ...
    def test_get_model_for_url__with_unknown_url(self):
        ...
        with self.assertRaises(TargetDoesNotExist):
            util.get_model_for_url_path(
                functions.url(constants.view_all_endpoints, bad_path)
            )
    ...
    def test_get_model_for_url__with_unknown_slug(self):
        ...
        with self.assertRaises(TargetDoesNotExist):
            util.get_model_for_url_path(
                functions.url(constants.correct_config, "unknown-slug")
            )
