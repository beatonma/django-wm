...
from mentions.tests.views.mentionable import AllEndpointsMentionableTestView
from mentions.tests import constants
from mentions.tests.constants import views
...
urlpatterns += util.correct_urlpatterns + [
    re_path(
        fr'^{bad_modelname_key}/{views.all_endpoints}/'
        fr'(?P<slug>[\w\-.]+)/?$',
        AllEndpointsMentionableTestView.as_view(),
        kwargs={
            'model_name_with_mistyped_key': 'tests.MentionableTestStub',
        },
        name=f'bad_modelname_key_{views.all_endpoints}'),
    re_path(
        fr'^{bad_modelname_value}/{views.all_endpoints}/'
        fr'(?P<slug>[\w\-.]+)/?$',
        AllEndpointsMentionableTestView.as_view(),
        kwargs={
            'model_name': 'tests.UnresolvableModel',
        },
        name=f'bad_modelname_value_{views.all_endpoints}'),
]
...
class GetModelForUrlTests(TestCase):
    ...
    def test_get_model_for_url__with_unknown_url(self):
        ...
        with self.assertRaises(TargetDoesNotExist):
            get_model_for_url(util.url(views.all_endpoints, bad_path))
    ...
    def test_get_model_for_url__with_correct_slug(self):
        ...
        retrieved_object = get_model_for_url(
            util.url(constants.correct_config, views.all_endpoints, self.slug))
    ...
    def test_get_model_for_url__with_bad_model_name_config(self):
        ...
        with self.assertRaises(BadConfig):
            get_model_for_url(
                util.url(bad_modelname_key, views.all_endpoints, self.slug))

        # Raise BadConfig if `model_name` kwarg has bad/unresolvable path.
        with self.assertRaises(BadConfig):
            get_model_for_url(
                util.url(bad_modelname_value, views.all_endpoints, self.slug))
